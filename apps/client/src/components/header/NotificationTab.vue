<template>
  <v-menu :close-on-content-click="false" offset="6, 0">
    <template v-slot:activator="{ props }">
      <v-btn
        icon
        class="ml-1 text-secondary ml-sm-2"
        color="darkText"
        rounded="sm"
        size="small"
        v-bind="props"
    ...
    <v-sheet rounded="md" width="387" class="notification-dropdown">
      <div class="pa-4">
        <div class="d-flex align-center justify-space-between">
          <h6 class="mb-0 text-subtitle-1">Notifications</h6>
          <v-btn
            variant="text"
            color="success"
            icon
      ...
      <perfect-scrollbar style="height: calc(100vh - 300px); max-height: 265px">
        <v-list class="py-0" lines="two" aria-label="notification list" aria-busy="true">
          <!-- <v-list-item value="1" color="secondary" class="py-1 no-spacer" :active="isActive">
            <template v-slot:prepend>
              <v-avatar
                size="36"
                variant="flat"
                color="lightsuccess"
                class="py-2 mr-3 text-success"
              >
                <GiftOutlined />
              </v-avatar>
            </template>
            <div class="d-inline-flex justify-space-between w-100">
              <h6 class="mb-0 text-subtitle-1 font-weight-regular">
                It's <span style="font-weight: 600">Cristina danny's</span> birthday today.
              </h6>
              <span class="text-caption">3:00 AM</span>
            </div>

            <p class="my-0 text-caption text-medium-emphasis">2 min ago</p>
          </v-list-item>
          <v-divider></v-divider>
          <v-list-item value="2" color="secondary" class="no-spacer">
            ...
            <p class="my-0 text-caption text-medium-emphasis">5 August</p>
          </v-list-item>
          <v-divider></v-divider>
          <v-list-item value="3" color="secondary" class="no-spacer" :active="isActive">
            <template v-slot:prepend>
              <v-avatar size="36" variant="flat" color="lighterror" class="py-2 mr-3 text-error">
                <SettingOutlined />
              </v-avatar>
            </template>
            <div class="d-inline-flex justify-space-between w-100">
            ...
            <p class="my-0 text-caption text-medium-emphasis">7 hours ago</p>
          </v-list-item>
          <v-divider></v-divider>
          <v-list-item value="4" color="secondary" class="no-spacer">
            ...
            <p class="my-0 text-caption text-medium-emphasis">Daily scrum meeting time</p>
          </v-list-item> -->
        </v-list>
      </perfect-scrollbar>
      <v-divider></v-divider>
...
<style>
.v-tooltip :deep(.v-overlay__content.custom-tooltip) {
  padding: 2px 6px;
}
</style>
